...
public class Client {
    ...
    private static final Queue<KeyValuePacket> failedMessage = new LinkedBlockingQueue<>();
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                packet = new KeyValuePacket(Type.PUT, dataList.get(i).getKey(), dataList.get(i).getValue());
                log.info("Sent PUT request to server No. " + serverNo + ": key: " + dataList.get(i).getKey() + ", value: " + dataList.get(i).getValue());
    ...
    public static void parseServerList(Map<String, String> data) throws NumberFormatException {
        data.forEach((key, value) -> serverList.put(Integer.parseInt(String.valueOf(key.charAt(key.length() - 1))), Map.entry(value.split(":")[0], Integer.parseInt(value.split(":")[1]))));
    }
    ...
    public static void userInterface() throws IOException, NotBoundException {
        ...
        // user interaction
        while (true) {
            log.info("We have " + serverList.keySet().size() + " servers are ready for you: " + Arrays.toString(serverList.keySet().toArray()));
            ...
            try {
                ...
                if ("get".equalsIgnoreCase(input)) {
                    ...
                    log.info(prefix + ": " + api.get(new KeyValuePacket(Type.GET, key, null)));
                ...
                log.error("Failed to process the request, the system will retry the request with another available server");
                // add to failure queue
                failedMessage.add(packet);
            }
        }
    ...
    public static void monitor() {
        ...
        executorService.scheduleAtFixedRate(() -> {
            ...
            while (!failedMessage.isEmpty()) {
                KeyValuePacket packet = failedMessage.poll();
