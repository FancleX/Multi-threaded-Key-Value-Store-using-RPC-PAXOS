    ...
    public synchronized String propose(KeyValuePacket clientMessage) {
        ...
        serverList.forEach((key, val) -> {
            ...
            try {
                ...
                log.error("Cannot connect to server with hostname: " + hostname + ", port: " + port + " at prepare phase");
            }
        });
        // fail
        if (isDown) {
        ...
        if (promises.size() > (serverList.size() / 2)) {
            ...
            serverList.forEach((key, val) -> {
                ...
                try {
                    ...
                    log.error("Cannot connect to server with hostname: " + hostname + ", port: " + port + " at accept request phase");
                }
            });
        } else {
        ...
        if (acks > (serverList.size() / 2)) {
            ...
            serverList.forEach((key, val) -> {
                ...
                try {
                    ...
                    log.error("Cannot connect to server with hostname: " + hostname + ", port: " + port + " at commit phase");
                }
            });
        } else {
        ...
        serverList.forEach((key, val) -> {
            ...
            try {
                ...
                log.error("Cannot connect to server with hostname: " + hostname + ", port: " + port + " at paxos end phase");
            }
        });
